...
from handrail_generator import build_handrail
...
def _flight_treads_risers(steps, going, rise, width, tread_t, riser_t, nosing=0.0):
    ...
    for i in range(steps):
        ...
        risers.append(r.translate((x0, 0, i * rise)))
...
def _flight_stringers(steps, going, rise, width, str_depth, str_width, tread_t=20.0, riser_t=20.0, nosing=20.0):
    ...
    """
    stringers = []
    # Inner stringer: at Y=0, extruded -Y ÔåÆ Y from 0 to -str_width
...
def _winder_corner_stringers(volumetric, winder_width, str_width, pivot_global):
    ...
    # Slab sits just inside the outer boundary
    x_span = bb.max.X - bb.min.X + 400
    bottom_slab = Box(x_span, str_width, z_span,
                      align=(Align.CENTER, Align.CENTER, Align.CENTER))
    ...
    y_span = bb.max.Y - bb.min.Y + 400
    right_slab = Box(str_width, y_span, z_span,
                     align=(Align.CENTER, Align.CENTER, Align.CENTER))
...
def build_structural_staircase(config):
    ...
    vol_config["unified_soffit"] = True
    volumetric = build_staircase(vol_config)

    # -----------------------------------------------------------------------
    # 1. BOTTOM FLIGHT  ÔÇö +X, inner edge at Y=-inner_r
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 2. WINDER ÔÇö pivot at (pivot_x, pivot_y)
    # -----------------------------------------------------------------------
    ...
    # Winder corner stringers (sliced from volumetric ÔåÆ stepped profiles)
    ...
    # -----------------------------------------------------------------------
    # 3. TOP FLIGHT ÔÇö rotated 90┬░, inner edge at X=pivot_x+inner_r
    # -----------------------------------------------------------------------
    ...
    all_ribs = [p for p in all_ribs if p and len(p.solids()) > 0]

    print(f"  Total: {len(all_treads)} treads, {len(all_risers)} risers, "
          f"{len(all_stringers)} stringers, {len(all_carriages)} carriages, "
          f"{len(all_ribs)} ribs, 1 plaster shell")

    # -----------------------------------------------------------------------
    # 6. ARCHITECTURAL ELEMENTS (Handrail & Balusters)
    # -----------------------------------------------------------------------
...
def display_structural(elements):
    ...
    _add("baluster", elements["balusters"], C_BALUSTER, 1.0)
